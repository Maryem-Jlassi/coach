...
class Config:
    ...
    ELEVENLABS_VOICE_ID = "21m00Tcm4TlvDq8ikWAM"  # Rachel voice - warm female voice
    TTS_LANGUAGE = "en"
    ...
    def load_athlete_profile(self, session_id: str) -> Optional[AthleteProfile]:
        ...
        if row:
            return AthleteProfile(
                ...
                data_collection_complete=row[12]
            )
        return None
...
class ConversationalVoiceCoach:
    ...
    def create_conversational_agent(self, context: str = "") -> Agent:
        ...
        return Agent(
            ...
            goal='Have natural, brief conversations like a supportive friend',
            backstory=f'''You are Coach Alex, a warm and friendly mental performance coach who talks 
            like a real person having a casual conversation. You keep your responses SHORT and conversational.
            
            KEY COMMUNICATION STYLE:
            - Maximum 1-2 sentences per response
            - Natural, friendly tone like talking to a friend
            - Ask ONE question at a time
            - Use conversational language ("Great!", "Awesome!", "I see")
            ...
            This is like texting or instant messaging - short, friendly, and to the point.
            ''',
            llm=self.llm,
            verbose=False,
    ...
    def start_vocal_data_collection(self, profile: AthleteProfile) -> Dict[str, Any]:
        ...
        # Welcome message
        welcome_text = "Hi there! I'm Coach Alex, your mental performance coach. What's your name?"
    ...
    def _get_next_conversation_step(self, profile: AthleteProfile, current_step: str, user_input: str) -> tuple[str, str]:
        ...
        step_responses = {
            'name': (
                'age',
                f"Nice to meet you, {profile.name}! How old are you?"
            ),
            'age': (
                'sport',
                f"Great! And what sport do you focus on, {profile.name}?"
            ),
            'sport': (
                'level',
            ...
            'level': (
                'training_frequency', 
                f"Perfect! How many times do you typically train per week?"
            ),
            'training_frequency': (
                'goals',
            ...
            'goals': (
                'current_feeling',
                f"That's a great goal! How are you feeling about your performance lately?"
            ),
            'current_feeling': (
                'confirmation',
                f"Thanks for sharing that! Let me quickly confirm your info..."
            ),
            'confirmation': (
                'complete',
        ...
        if current_step in step_responses:
            ...
            return 'complete', "Perfect! Let's start your mental performance session!"
    ...
    def generate_coaching_question(self, profile: AthleteProfile, question_context: str = "") -> Dict[str, Any]:
        ...
        task = Task(
            ...
            REQUIREMENTS:
            - Ask ONE specific question (maximum 20 words)
    ...
    def generate_coaching_response(self, profile: AthleteProfile, user_response: str, conversation_history: List[Dict]) -> Dict[str, Any]:
        ...
        task = Task(
            ...
            EXAMPLES:
            - "That makes sense! Try focusing on your breathing when that happens."
            - "I hear you. Remember, nerves show you care - that's actually good!"
            - "Great awareness! Quick visualization before training can help with that."
    ...
    def generate_final_coaching_summary(self, profile: AthleteProfile, conversation_history: List[Dict]) -> Dict[str, Any]:
        ...
        task = Task(
            ...
            3. ONE specific technique to practice (2 sentences max)
            4. Encouraging closing (1 sentence)
            
            TOTAL: Maximum 50 words. Keep it actionable and memorable!
            
            Example structure:
            "Great conversation, {profile.name}! I can see you're really committed. 
    ...
    def _build_conversation_context(self, profile: AthleteProfile, history: List[Dict]) -> str:
        ...
        for exchange in history[-5:]:  # Last 5 exchanges
            ...
            context += f"A: {exchange.get('response', '')}\n"
...
def show_conversation_interface():
    ...
    for exchange in st.session_state.conversation_history:
        ...
        <div class="coach-message">
            <strong>🎤 Coach Alex:</strong> {exchange['coach_message']}
        </div>
        ''', unsafe_allow_html=True)
...
def handle_data_collection_step():
    ...
    if f'step_{current_step}_message' not in st.session_state:
        if len(st.session_state.conversation_history) > 0:
            ...
            last_response = st.session_state.conversation_history[-1].get('user_response', '')
...
def add_to_conversation(user_response: str, next_step: str):
    ...
    # Get current coach message
    current_message = ""
    if st.session_state.conversation_history:
        current_message = st.session_state.conversation_history[-1].get('coach_message', '')
    elif 'welcome_message' in st.session_state:
        current_message = st.session_state.welcome_message['message']
    
    # Add to conversation history
    exchange = {
        'coach_message': current_message,
        'user_response': user_response,
        'timestamp': datetime.now().isoformat()
    }
    ...
    if st.session_state.conversation_history:
        with st.expander("📝 Conversation Summary"):
            for i, exchange in enumerate(st.session_state.conversation_history):
                ...
                st.markdown(f"Coach: {exchange.get('coach_message', 'N/A')}")
                st.markdown(f"You: {exchange.get('user_response', 'N/A')}")
                if exchange.get('coach_response'):
                    st.markdown(f"Coach Response: {exchange['coach_response']}")
                st.markdown("---")
...
def test_elevenlabs_connection():
    ...
    if Config.ELEVENLABS_API_KEY == "your_elevenlabs_api_key_here":
        ...
        if st.sidebar.button("Test ElevenLabs"):
            try:
                coach = st.session_state.voice_coach
                test_audio = coach.voice_service.text_to_speech_elevenlabs("Test message")
                if test_audio:
...
if __name__ == "__main__":
    ...
    with col1:
        ...
        try:
            ...
            st.success("✅ Ollama Ready") if response.status_code == 200 else st.error("❌ Ollama Offline")
        except:
    ...
    with col2:
        ...
        if Config.ELEVENLABS_API_KEY != "your_elevenlabs_api_key_here":
            ...
            st.warning("⚠️ ElevenLabs Not Configured")
    ...
    with col3:
        ...
        try:
            ...
            st.error("❌ Speech Recognition Missing")
